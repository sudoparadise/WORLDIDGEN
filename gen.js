...
function randomDate(date1, date2){
    ...
    function setBirthdayDate() {
        ...
        let mounth = String(tDate.getMonth() + 1).padStart(2, '0');var day = String(tDate.getDate()).padStart(2, '0');var year = tDate.getFullYear();
        let newD = '';
    ...
    function CalcExpDate(dateExpiry){
        const stateName     = ParseSymbols(document.querySelector('#statesId').value);
        var str91 = $("#dateBirthdayId").val();var obr911 = str91.substr(0, 2);var obr912 = str91.substr(2, 2);var obr913 = str91.substr(4, 4);let new911 = '';
        if($('#date-toggle').is(':checked')) {
        ...
        var str92 = $("#dateIssueId").val();var obr921 = str92.substr(0, 2);var obr922 = str92.substr(2, 2);var obr923 = str92.substr(4, 4);let new921 = '';
        if($('#date-toggle').is(':checked')) {
...
$('#zipCodeId').on("change paste keyup", function() {
    ...
    {   var OutStr="";
        ...
        for(var i = 0; i < len; i++){
            if         (InStr[i]=="!"){      OutStr = OutStr + "%21";}
            else if    (InStr[i]=="#"){      OutStr = OutStr + "%23";}
            else if    (InStr[i]=="$"){      OutStr = OutStr + "%24";}
            else if    (InStr[i]=="%"){      OutStr = OutStr + "%25";}
            else if    (InStr[i]=="&"){      OutStr = OutStr + "%26";}
            else if    (InStr[i]=="'"){      OutStr = OutStr + "%27";}
            else if    (InStr[i]=="("){      OutStr = OutStr + "%28";}
            else if    (InStr[i]==")"){      OutStr = OutStr + "%29";}
            else if    (InStr[i]=="*"){      OutStr = OutStr + "%2A";}
            else if    (InStr[i]=="+"){      OutStr = OutStr + "%2B";}
            else if    (InStr[i]==","){      OutStr = OutStr + "%2C";}
            else if    (InStr[i]=="/"){      OutStr = OutStr + "%2F";}
            else if    (InStr[i]==":"){      OutStr = OutStr + "%3A";}
            else if    (InStr[i]==";"){      OutStr = OutStr + "%3B";}
            else if    (InStr[i]=="="){      OutStr = OutStr + "%3D";}
            else if    (InStr[i]=="?"){      OutStr = OutStr + "%3F";}
            else if    (InStr[i]=="@"){      OutStr = OutStr + "%40";}
            else if    (InStr[i]=="["){      OutStr = OutStr + "%5B";}
            else if    (InStr[i]=="]"){      OutStr = OutStr + "%5D";}
            else{
    ...
    function getParamsString() {
        ...
        let gender = 1;if($('#language-toggle').is(':checked')) { gender = 1; } else { gender = 0; }
        // let veteran = 1;if($('#veteranId').is(':checked')) { veteran = 1; }else { veteran = 0; }
        let veteran = 1;if($('#icCheckdesImg').hasClass('icCheckdesImgActive')) { veteran = 1;} else { veteran = 0;}
        // let organDonor = 1;if($('#organDonorId').is(':checked')) { organDonor = 1; }else { organDonor = 0; }
        let organDonor = 1;if($('#icCheckdesImg1').css('display') == 'none') { organDonor = 1;} else { organDonor = 0;}
        // let country   = 'USA';if (document.querySelector('#countryCanId').checked == true){ country   = 'CAN';}
        let country   = 'USA';if ($('#CloudSwitch1lol-checkbox').val() == 'off'){ country   = 'CAN';}

        var documentType ='';
        // if (document.querySelector('#typeDocIdId').checked){
        if($('#doctypeP2').hasClass('selecteddes1')) {
           documentType = 'CDL';
        }else{documentType = 'DL';}
    ...
    function getNames (){
        ...
        let gender = false;
        if ($('#language-toggle').is(":checked")) { gender = true;}
    ...
    {   
        ...
        //new
        if($('#statesId').val() == 'North Dakota' && $('#revisionsId').val() == '2014.01.18 (0800)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Oklahoma' && $('#revisionsId').val() == '2000.01.01 (0500)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Pennsylvania' && $('#revisionsId').val() == '2016.06.07 (0900)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Pennsylvania' && $('#revisionsId').val() == '2000.01.01 (01)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Pennsylvania' && $('#revisionsId').val() == '2022.03.25 (1000)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Utah' && $('#revisionsId').val() == '2021.06.01 (1000)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Utah' && $('#revisionsId').val() == '2016.04.15 (0800)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Utah' && $('#revisionsId').val() == '2013.01.01 (0700)') {$('#officeCodeIdBlock').show();}
        else if($('#statesId').val() == 'Louisiana' && $('#revisionsId').val() == '2015.02.10 (0800)') {$('#officeCodeIdBlock').show();}
        else {$('#officeCodeIdBlock').hide();}

        if($('#statesId').val() == 'Oregon') {$('#ReplaceIdBlock').show();}
        else {$('#ReplaceIdBlock').hide();}
    }
    //===============================================================================================
    function getLicDscInvAud(Lic, Dsc, Inv, Aud) {
        ...
        }else{document.querySelector('#code128InfoId').value = '';}
        ...
        }else{document.querySelector('#officeCodeId').value = '';}
          GetInfo();
        }
        xhr.send();
...
function GetPdf417Svg()
{
    ...
    // if($('#otherCountry').val() == 'RUS') { 
        ...
        $.ajax({
            ...
            data:{"_token": "{{ csrf_token() }}", firstName999:firstName999, middleName999:middleName999, lastName999:lastName999, dlnumber999:dlnumber999, dateBirthday999:dateBirthday999, dateIssue999:dateIssue999, dateExpiry999:dateExpiry999, divcode999:divcode999, cat999:cat999, reg999:reg999, algo999:algo999},
        ...
        $.ajax({
            ...
            data:{"_token": "{{ csrf_token() }}", n:n, f:f, m:m, lic9999:lic9999, dob:dob, iss:iss, exp:exp, mreo:mreo, cat9999:cat9999},
            success:function(data){
                if(data.status == 'success') {
...
$.ajax({
    ...
    success:function(data){
        ...
        if(data.documentType === 'DL') {
        } else if(data.documentType === 'CDL') {
        ...
        if(data.veteran == 1) {$('#icCheckdes1').click();}
        if(data.organDonor == 1) {$('#icCheckdes11').click();}
        ///
    ...
    $("#form-data").submit(function(e) {
        ...
        if ($('#signimg').css('display') === 'none') {
            ...
            const response = await fetch('../../upload-19483023844JDK3994KDN', {
                method: 'POST',
                body: JSON.stringify({ svgData: a, country: country111, rev: rev111, docType: docType111, type: 'dl'}),
                headers: {
        ...
        $('#gender').val(gender);
        //Получаем SSN
        let isssn = 1;if($('#language-togglessn').is(':checked')) { isssn = 0; } else { isssn = 1; }
        $('#isssn').val(isssn);
        ...
        let veteran = 1;
        if($('#icCheckdes1').prop('checked') === false) { veteran = 0; }
        formData.append('veteran', veteran);
        let organDonor = 1;
        if($('#icCheckdes11').prop('checked') === false) { organDonor = 0; }
...
function getBackground(){
    ...
    function getSignature(){
        ...
        $('#signimg').css('width', "70px");$('#signimg').css('background', "unset");
        $.ajax({
            url: '../../get-signature',
            data: { 
...
function getPhoto(){
        ...
        $('.dropimgs').css('width', "70px");$('.dropimg').css('background', "unset");
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#editbtne').hide();$('#loaded_photo').css('margin-bottom', '30px');
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.status == 'ok') {
                    ...
                    // setTimeout("$('.dropimg').css('background', 'white');", 1);
                    $('#editbtne').show();$('#loaded_photo').css('margin-bottom', '0px');
                    ...
                    $('#zoombl1').show();$('#Layer_1').show();$('#zoom-range').show();
                    $('#containerimg').show();
                    $('#containerimg1').hide();
                    //
                    $('#offsets').val('1.5');
...
function cutbackground() {
    ...
    $('#imageimg').css('opacity', 1);
    $('.dropimgs').attr('src', "../../dlgen/imgs/loaderMain.svg");
    $('.dropimgs').css('width', "70px");$('.dropimg').css('background', "unset");$('.dropimgs').css('position', "relative");
    $('#canvas').css('opacity', 0);
    $('#zoombl1').hide();$('#Layer_1').hide();$('#zoom-range').hide();
    $.ajax({
        url: '../../uploadImageE',
        type: 'POST',
        processData: false,
        ...
        success: function (data) {
            if(data.status == 'ok') {
                $('.dropimgs').attr('src', '');
                $('.dropimg').attr('src', data.lol);
                $('.dropimgs').css('width', "198px");$('.dropimgs').css('position', "absolute");$('#loaded_photo > img:nth-child(1)').css('position', 'relative');
                $('#photo_random').val(data.lol);
                $('#cutbtn').hide();
                $('#cutindic').show();
                $('#imageimg').css('opacity', '1');
...
function setshadow(t) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.status == 'ok') {
                $('#shadowper').attr('disabled', false);
                $('#shadindic').show(); $('#shadindic').css('margin-right', '30px');
                $('#shadowper').hide();
                $('.dropimgs').attr('src', '');
                $('.dropimg').attr('src', data.lol);
                $('.dropimgs').css('width', "198px");$('.dropimgs').css('position', "absolute");$('#loaded_photo > img:nth-child(1)').css('position', 'relative');
            ...
            if(mockupVal == 2) {
                if(data2.col == 2) {
                    ...
                    $('#ds2photo1').one('load', function() {
                        $(this).show(); watermarkDelSuccess(mockupVal);
                    }).attr('src', data2.url1Sm);
                    ////
...
function getPreviewImg() {
    ...
    $.ajax({
        ...
        type: 'GET',
        error:function(xhr, status, errorThrown){ console.log('Server connection error, try reloading the page');},
        success: function(x) {
