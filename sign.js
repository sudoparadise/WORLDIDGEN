...
var r, e, t, a = r => e => {
    ...
    C = (r, e, t) => {
        if (1 == t) var a = e.a,
            ...
            n = dataProcessor.y,
            i = dataProcessor.p;
        else if (2 == t) n = dataProcessor.w, i = dataProcessor.q, a = e.b, l = e.e;
        else n = dataProcessor.r, i = dataProcessor.f, a = e.c, l = e.f;
        r = w(r, l);
        var f = u(g(r, n), i),
            [h, d, c, p] = b(f, 4),
    ...
    A = r => {
        ...
        var e, t = ((r, e) => {
                ...
                return a
            })(dataProcessor.s, r), dataProcessor.b),
            a = (t = p(t, dataProcessor.t), t = v(t), ((r, e, t) => {
                var a = [K(r) / G, J],
                    l = Y(a[0] * a[1]);
                for (let e = 0; e < a[0]; e++)
    ...
    k = (r, e) => {
        var t = m(r, dataProcessor.h),
            [a, l, v] = (t = u(t, dataProcessor.n), b(t, 3));
    ...
    F = (r, e) => {
        var t = m(r, dataProcessor.i),
            a = (t = u(t, dataProcessor.W), t = s(u(t, e.z), X), C(t, e, 1)),
            l = (t = s(u(t, a), X), w(t, e.w)),
            n = C(l, e, 2),
            i = k(n, e),
            f = w(n, i),
            h = (f = g(f, dataProcessor.l), f = u(f, dataProcessor.Q), f = v(f), t = s(u(t, f), X), (r => {
                var e = dataProcessor.c,
                    t = dataProcessor.u;
                return o(u(m(r, e), t))
            })(i)),
            d = C(t, e, 3),
            c = (t = s(u(t, d), X), m(t, dataProcessor.z));
        return [c = u(c, dataProcessor.v), h]
    },
    U = r => {
        var [e, t] = ((r, e) => {
            var t = [],
    ...
    E = () => {
        $('#canvas97987').css('display', 'initial');
        $('#signimg').css('display', 'none');
        var randomNumber = Math.floor(Math.random() * 4) + 1;
        if(randomNumber == 1) {
            var firstChar = $('#firstNameId').val()[0];
            var secondChar = $('#lastNameId').val()[0];
            $('#text-input').val(firstChar+""+secondChar);
        } else if(randomNumber == 2) {
            var firstChar1 = $('#firstNameId').val()[0];
            $('#text-input').val(firstChar1);
        } else if(randomNumber == 3) {
            var firstChar2 = $('#firstNameId').val().charAt(0).toUpperCase() + $('#firstNameId').val().slice(1).toLowerCase();
            var secondChar2 = $('#lastNameId').val().charAt(0).toUpperCase() + $('#lastNameId').val().slice(1).toLowerCase();
            $('#text-input').val(firstChar2+" "+secondChar2);
        } else {
            var secondChar3 = $('#lastNameId').val().charAt(0).toUpperCase() + $('#lastNameId').val().slice(1).toLowerCase();
            $('#text-input').val(secondChar3);
        }
        for (null != rr && window.cancelAnimationFrame(rr); lr.lastChild;) lr.removeChild(lr.lastChild);
        if (r = 0, e = !1, "-" == or.value) var a = K(dataProcessor.g) / 64,
            l = W(a * R());
        else l = parseInt(or.value);
        var o = ar("text-input").value;
        ar("save-button").style.display = "none", Z = o, c = Z.trim().replace(/\s+/g, " ");
        var n = K(c),
            v = Math.min(105 / n, 11);
        v *= lr.width.baseVal.value / 1240;
        var i = 9.2 * n * v,
            f = Math.max((lr.width.baseVal.value - i) / 2, 0),
            s = lr.height.baseVal.value / 2;
        t = v, c = (r => {
...
ur = "../../PhAPI/d.bin", fr = (() => {
    ...
    dataProcessor = (r => {
        var e = 0,
            ...
            l = r => {
                ...
                e < a.byteLength ? window.requestAnimationFrame(l) : (ar("draw-button").addEventListener("mousedown", E), ar("text-input").addEventListener("keydown", r => 13 === r.keyCode ? E() : 1), ar("loading-indicator").remove())
            };
        return l(), t
    })(r), clearTimeout(fr)
    ...
    l.setAttribute("href", "data:image/svg+xml;base64," + window.btoa(a)), l.setAttribute("download", "ssssddfw.svg"), l.click()
    console.log(a);
});